...
<%
	...
	int randDate = MILLISEC_PER_DAY * randNum;

	String reservationDate = new SimpleDateFormat("yyyy.MM.dd").format(System.currentTimeMillis() + randDate);
%>
<html lang="ko">
...
<body>
				...
				<div class="section_booking_form">
					<div class="booking_form_wrap">
						<div class="form_wrap">
							...
							<div class="form-error-msg hide">
								<p class="emailCheck"></p>
							</div>

							<form method="post" action="./booking2" class="form_horizontal">
								...
								<div class="inline_form">
									...
									<div class="inline_control">
										<input type="email" name="reservationEmail" id="email"
											class="email" placeholder="crong@codesquad.kr"
											maxlength="50" required
											<c:if test="${not empty login }">readOnly value=${login.reservationEmail} </c:if>>
										<div class="warning_msg"></div>
									</div>
								</div>
								<div class="inline_form last">
									...
									<div class="inline_control">
										<p class="inline_txt selected">
											<span><%=reservationDate%></span>, 총 <span id="totalCount">0</span>매
										</p>
									</div>
								</div>
					...
					<div class="section_booking_agreement">
						...
						<div class="agreement">
							...
							</span> <a href="javascript:;" class="btn_agreement"> <span
	...
	<script type="rv-template" id="productInfo">
      <div class="store_details">
        ...
        <p class="dsc">
          장소 : {{displayInfo.placeStreet}} <br> 기간 : 2017.2.17.(금)~2017.4.18.(화)
        </p>
        ...
        <p class="dsc">
          ...
          {{#each (reverseArray productPrices)}} {{priceType priceTypeName}}:
            {{priceFormat price}}원 <br> 
          {{/each}}
        </p>
      </div>
    </script>
	<script type="rv-template" id="quantity">
      {{#each productPrices}}
        <div class="qty">
          ...
          <!-- [D] 수량이 최소 값이 일때 ico_minus3, count_control_input에 disabled 각각 추가, 수량이 최대 값일 때는 ico_plus3에 disabled 추가 -->
            <div class="clearfix">
              <a href="javascript:;"
                class="btn_plus_minus spr_book2 ico_minus3 disabled" title="빼기">
              </a> 
              <input type="tel" class="count_control_input disabled"
                      id={{productPriceId}} value="0" readonly title="수량"> 
              <a
                href="javascript:;" class="btn_plus_minus spr_book2 ico_plus3"
                title="더하기"> 
              </a>
            </div>
          ...
          <div class="qty_info_icon">
            ...
            <em class="product_dsc">{{priceFormat price}}원 ({{discountRate}}% 할인가) </em>
          </div>
        </div>
      {{/each}}
    </script>
